module GptMigrate
  class MigrationAgent < ApplicationAgent
    ...
    def write_migration(file, _external_dependencies, file_dependencies)
    ...
    def get_dependencies(file)
      ...
      if internal_dependencies.include?(file)
        Rails.logger.debug "Warning: #{file} seems to depend on itself. Automatically removing #{file} from the list of internal dependencies."
    ...
    def get_function_signatures(target_files = [])
      ...
      target_files.each do |target_file|
        unless (sigs = function_signatures[target_file])
    ...
    def add_env_files
      copy_files(globals.source_dir, globals.target_dir, EXCLUDED_FILES)
      ...
      external_deps_name, _, external_deps_content = llm_write_file(add_docker_requirements_prompt, globals.target_dir,
                                                                    nil)
